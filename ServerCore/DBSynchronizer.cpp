...
{
	// xml을 파싱을 먼저 해서 구조를 모두 분석해서 메모리 상에 들고 있음
	ParseXmlDB(path);

	// DB에 접근해서 query를 보내서 인덱스, sp정보도 긁어옴
	GatherDBTables();
	GatherDBIndexes();
	GatherDBStoredProcedures();

	// 둘의 정보를 비교해서 다르다면 create, update하는 등등
	// 먼저 예약 후
	CompareDBModel();
	// 뒤에 실제 실행
...
void DBSynchronizer::ParseXmlDB(const WCHAR* path)
{
	...
	{
		// 체크만 하는게 아니라 해당 객체를 만들어서 메모리에 들고 있게 됨
...
bool DBSynchronizer::GatherDBTables()
{
	...
	while (getDBTables.Fetch())
	{
		...
		DBModel::ColumnRef column = MakeShared<DBModel::Column>();
		{
			...
			// varchar같은 경우는 length같은게 ㅣㅍㄹ요할 수 있음
...
void DBSynchronizer::CompareDBModel()
{
	...
	for (DBModel::TableRef& dbTable : _dbTables)
	{
		...
		if (findTable != xmlTableMap.end()) //뭔가 다르다면 정보를 기입해서 업데이트 쿼리에 쿼리를 넣고 있음
		{
